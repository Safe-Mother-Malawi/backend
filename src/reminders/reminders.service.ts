...
export class RemindersService {
  ...
  /**
   * Replace all pending reminders for an appointment.
   * Creates 24-hour and 1-hour reminders for each target user.
   */
  async replaceAppointmentReminders(params: {
    appointmentId: string;
    userIds: string[];
    appointmentDate: string;
    ...
    await this.cancelPendingAppointmentReminders(params.appointmentId);
    ...
    const now = new Date();
    const reminderSpecs = [
      { label: '24 hours', scheduledFor: new Date(appointmentDateTime.getTime() - 24 * 60 * 60 * 1000) },
      { label: '1 hour', scheduledFor: new Date(appointmentDateTime.getTime() - 60 * 60 * 1000) },
    ].filter((spec) => spec.scheduledFor > now);
    ...
    for (const userId of userIds) {
      for (const spec of reminderSpecs) {
        const reminder = await this.create(userId, {
          ...
          body: this.buildAppointmentReminderBody({
            title: params.title,
  ...
  async cancelPendingAppointmentReminders(appointmentId: string): Promise<void> {
  ...
  private parseAppointmentDateTime(date: string, time?: string | null): Date | null {
    if (!date) return null;

    const trimmedTime = (time ?? '09:00').trim();
    ...
    const malawiUtcOffsetHours = 2;

    return new Date(Date.UTC(year, month - 1, day, hours - malawiUtcOffsetHours, minutes, 0, 0));
  }

  private buildAppointmentReminderBody(params: {
    title: string;
    patientName?: string | null;
    appointmentDate: string;
    appointmentTime?: string | null;
    ...
    const time = params.appointmentTime ? ` at ${params.appointmentTime}` : '';
    const location = params.location ? ` at ${params.location}` : '';
    const patient = params.patientName ? ` for ${params.patientName}` : '';
    return `${params.title}${patient} is in ${params.leadTime}: ${params.appointmentDate}${time}${location}.`;
  }
